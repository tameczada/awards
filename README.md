...
- **Painel admin** (`/admin.html`): login protegido, CRUD completo de categorias e opções,
  upload de imagem por categoria, **upload da imagem de fundo da página inicial**,
  edição do título/subtítulo do site, e apuração de votos em tempo real.
...
```
votacao-app/
├── server.js              # backend Express (todas as rotas)
