...
async function getLiveSnapshot() {
  ...
  const categories = (catsRes.data || []).map((cat) => {
    ...
    const options = (optionsByCategory[cat.id] || [])
      ...
      // ordem fixa (display_order), sem reordenar por votos — evita confusão
      // com o número do "!votar N" que corresponde à posição de cada opção
...
app.get('/api/admin/categories', authRequired, async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .order('created_at', { ascending: true });
  if (error) return res.status(500).json({ error: error.message });
  // marca qual categoria está em foco no dashboard agora, pra destacar
  // "ao vivo" na fila de espera do painel admin
  const { data: cfg } = await supabase.from('dashboard_config').select('focused_category_id').eq('id', 1).single();
  const liveId = cfg ? cfg.focused_category_id : null;
  res.json(data.map((c) => ({ ...c, status: computeStatus(c), options_count: c.options?.[0]?.count || 0, live: c.id === liveId })));
});
...
app.post('/api/admin/categories/:id/queue', authRequired, async (req, res) => {
  ...
  const { data: cat, error: catErr } = await supabase
    .from('categories')
    .update({ status: 'agendada', starts_at: null, ends_at: null, paused: false })
    .eq('id', id)
    .select()
    .single();
  if (catErr) return res.status(500).json({ error: catErr.message });
