(() => {
  ...
  function renderQueue() {
    const waiting = categoriesCache.filter((c) => c.status === 'agendada');
    if (!waiting.length) {
      els.queueList.innerHTML = `<div class="queue-empty">Nenhuma categoria esperando. Crie uma nova categoria pra ela entrar na fila.</div>`;
      return;
    }
    els.queueList.innerHTML = waiting.map((cat, idx) => {
      const thumb = cat.image_url ? `style="background-image:url('${cat.image_url}')"` : '';
      return `
        <div class="queue-card">
          <span class="queue-position">${idx + 1}º</span>
  ...
  async function startCategory(id) {
  ...
  function openCategoryModal(id) {
    ...
    if (id) {
      ...
      $('category-card-size').value = cat.card_size || 'medio';
    } else {
      els.modalTitle.textContent = 'Nova categoria';
      els.catForm.reset();
      $('category-id').value = '';
      $('category-order').value = 0;
      $('category-card-size').value = 'medio';
    }
  ...
  els.catForm.addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      card_size: $('category-card-size').value,
    };
  ...
  // =====================================================
  // DASHBOARD AO VIVO (link com token)
  // =====================================================
  function buildDashboardUrl(dashToken) {
    return `${window.location.origin}/dashboard.html?token=${dashToken}`;
  }

  async function loadDashboardLink() {
    try {
      const data = await api('/admin/dashboard-token');
      $('dashboard-link').value = buildDashboardUrl(data.token);
    } catch (err) {
      showToast(err.message, true);
    }
  }

  $('dashboard-link-copy').addEventListener('click', async () => {
    const input = $('dashboard-link');
    input.select();
    try {
  ...
  $('dashboard-token-regenerate').addEventListener('click', async () => {
    if (!confirm('Isso invalida o link atual (qualquer fonte de OBS usando ele para de funcionar). Continuar?')) return;
    try {
      const data = await api('/admin/dashboard-token/regenerate', { method: 'POST' });
      $('dashboard-link').value = buildDashboardUrl(data.token);
  ...
  // =====================================================
  let twitchConfigCache = {};

  function updateTwitchStatusBadge(live) {
    const badge = $('twitch-status-badge');
    const text = $('twitch-status-text');
    badge.classList.remove('connected', 'error');
    if (live && live.connected) {
      badge.classList.add('connected');
      text.textContent = `conectado ao canal ${live.channel}`;
    } else if (live && live.lastError) {
      badge.classList.add('error');
      text.textContent = `desconectado (${live.lastError})`;
    } else {
  ...
  async function loadTwitchConfig() {
    try {
      const data = await api('/admin/twitch-config');
      twitchConfigCache = data;
      $('twitch-username').value = data.bot_username || '';
      $('twitch-channel').value = data.channel_name || '';
      $('twitch-token').placeholder = data.has_token ? 'já configurado — deixe em branco pra manter' : 'cole o token OAuth aqui';
      $('twitch-reply').checked = !!data.reply_in_chat;
      updateTwitchStatusBadge(data.live);
    } catch (err) {
      showToast(err.message, true);
    }
  }

  $('twitch-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const msg = $('twitch-form-msg');
    msg.textContent = '';
    const payload = {
      bot_username: $('twitch-username').value.trim(),
      channel_name: $('twitch-channel').value.trim().replace(/^@/, ''),
      reply_in_chat: $('twitch-reply').checked,
    };
    const tokenVal = $('twitch-token').value.trim();
    if (tokenVal) payload.oauth_token = tokenVal;

    try {
      const data = await api('/admin/twitch-config', { method: 'PUT', body: JSON.stringify(payload) });
      twitchConfigCache = { ...twitchConfigCache, ...data };
      $('twitch-token').value = '';
      $('twitch-token').placeholder = data.has_token ? 'já configurado — deixe em branco pra manter' : 'cole o token OAuth aqui';
  ...
  $('twitch-connect-btn').addEventListener('click', async () => {
    const btn = $('twitch-connect-btn');
    btn.disabled = true;
    btn.textContent = 'Conectando…';
    try {
      const data = await api('/admin/twitch-config/connect', { method: 'POST' });
      updateTwitchStatusBadge(data.live);
      showToast('Bot conectado ao chat!');
    } catch (err) {
      showToast(err.message, true);
    } finally {
      btn.disabled = false;
  ...
  $('twitch-disconnect-btn').addEventListener('click', async () => {
    try {
      await api('/admin/twitch-config/disconnect', { method: 'POST' });
      updateTwitchStatusBadge({ connected: false });
      showToast('Bot desconectado');
    } catch (err) {
      showToast(err.message, true);
    }
  });
