...
- **WebSocket funciona normalmente no Render**, inclusive no plano Free, sem
  configuração extra — o dashboard ao vivo e o bot da Twitch usam a mesma porta HTTP
  do serviço.
- **Mas**: quando o serviço "dorme" por inatividade, a conexão do bot da Twitch e
  qualquer aba/OBS conectado ao dashboard caem junto. O dashboard reconecta sozinho
  assim que o serviço acorda (alguém acessando qualquer página do site já é
  suficiente pra acordar), mas o bot da Twitch só volta a conectar quando o processo
  reinicia — o que só acontece se alguém acessar o site nesse meio-tempo. Pra chat
  da Twitch funcionando sem interrupção durante uma live inteira, considere um plano
  pago do Render (sem sleep) ou um serviço externo de "ping" a cada poucos minutos.
